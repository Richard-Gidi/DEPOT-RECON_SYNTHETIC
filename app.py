...
def write_stock_balance_sheet(ws, balance_data: list, sheet_type: str, month_label: str):
    ...
    subtitle = (
        "Balance carried forward at start of month"
...
if uploaded:
    ...
    with tab3:
        ...
        - **Opening Balance** = the *Balance b/fwd* value at the start of the selected month.
        ...
        if st.button("🔄 Fetch OILCORP Stock Balances", type="primary"):
            ...
            for depot_name in depots_to_query:
                ...
                for product in products_to_query:
                    ...
                    icon = "✅" if result["opening"] is not None else "⚠️"
                    log_lines.append(
                        f"{icon} {depot_name} [{product}] — "
                        f"Open: {result['opening']:,.0f}" if result["opening"] is not None
                        else f"⚠️ {depot_name} [{product}] — {result['error']}"
                    )
                    log_box.markdown(
